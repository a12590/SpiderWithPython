...
def impute_categories_missing_data(dataframe, column):
    """填充类别属性的缺失值"""
    most_common = Counter(dataframe[column].tocdlist()).most_common(1)[0][0]
...
def impute_df_numerical_but_cate_missing_data(dataframe, set_len_threshold=10, impute_value=-1):
    ...
    for c in columns:
        ...
        if set_len <= set_len_threshold:
            ...
            dataframe[c] = dataframe[c].astype(np.uint8)


def omit_column(dataFrame, dorp_columns):
    """
    忽略指定的字段
    :param dataFrame:
    :param dorp_columns:
    :return:
    """
    dataFrame.drop(dorp_columns,axis=1,inplace=True)
    return dataFrame
